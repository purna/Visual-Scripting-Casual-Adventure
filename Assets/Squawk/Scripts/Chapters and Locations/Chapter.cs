...
public class Chapter : MonoBehaviour
{
    ...
    [HideInInspector]
    public CutsceneController cutscenes; //references to any cutscenes contained in separate unity scenes
    [HideInInspector]
    ...
    private void Start()
    {
        ...
        //check there's only one cutscene controller
        if (FindObjectsOfType<CutsceneController>().Length > 1)
        {
            Debug.LogError("Should be only one CutsceneController");
        }
        else
        {
            cutscenes = FindObjectOfType<CutsceneController>();
        }        
    }
    ...
    public void InitialiseChapter(GameController _gc)
    {
        ...
        //cutscenecontroller is optional, so check whether it exists first.
        if (cutscenes != null)
        {
            cutscenes.Initialise(this);
        }
